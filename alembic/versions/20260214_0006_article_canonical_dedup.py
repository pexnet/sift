...
def upgrade() -> None:
    with op.batch_alter_table("articles", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column("dedup_confidence", sa.Float(), nullable=False, server_default=sa.text("1.0"))
        )
        batch_op.create_index(batch_op.f("ix_articles_canonical_url_normalized"), ["canonical_url_normalized"], unique=False)
