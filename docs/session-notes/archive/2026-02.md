...
### Local VictoriaMetrics/VictoriaLogs Stack Bootstrap (2026-02-23)

- Added full local observability stack scaffolding under `ops/observability/`:
  ...
  - Vector log shipping pipeline: `ops/observability/vector/vector.yaml`
  - startup/verification guide: `ops/observability/README.md`
- Stack coverage:
  - metrics scrape + storage for Sift API/scheduler/worker endpoints
  - log shipping from Sift Docker services (`app`, `scheduler`, `worker`) into VictoriaLogs
- Updated reference docs:
  - `docs/observability-runbook.md` now points to `ops/observability/README.md`
  - `docs/architecture.md` now documents optional local observability overlay services
  - `AGENTS.md` queue/scheduler section now includes observability stack bootstrap references

### Verification (2026-02-23, Observability Stack Bootstrap)

- `docker compose -f docker-compose.yml -f ops/observability/docker-compose.observability.yml config`
- Runtime validation note:
  - Docker daemon was unavailable in this environment for container launch/`vector validate`, so startup commands are
    documented but were not executed end-to-end in-session.
