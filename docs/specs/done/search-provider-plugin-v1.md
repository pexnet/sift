...
- Scope delivered:
  - runtime capability contract + registry validation
  - authenticated search-provider APIs (`GET /api/v1/search/providers`, `POST /api/v1/search/feeds`)
  - ordered fallback (`searxng` -> `brave_search` by config)
...
This document is archived as completed implementation history for the search-provider v1 slice.
...
## Implemented Checkpoint (2026-03-01 Slice 4: Provider Hardening Closeout)

1. Provider adapters now classify and return warning-coded failures for timeout/network/HTTP status/invalid JSON/payload.
2. Search service now preserves provider warning codes into API `warning_details` metadata.
3. Runtime adapters now normalize URL candidates and de-duplicate provider results by URL.
4. Added dedicated runtime hardening tests for:
   - timeout handling
   - HTTP 429 mapping
